...
function removeExport(filePath, exportName, exportType) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!inExport) {
      const exportPattern = new RegExp(`export\\s+(const|function|type|interface)\\s+${exportName}[\\s(:=]`);
...
for (const [filePath, exports] of Object.entries(REMOVALS)) {
  ...
  for (const exp of exports) {
    const success = removeExport(filePath, exp.name, exp.type);
