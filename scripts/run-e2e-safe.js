...
function resolveBin(bin) {
  const local = path.join(__dirname, '..', 'node_modules', '.bin', bin + (process.platform === 'win32' ? '.cmd' : ''));
...
async function main() {
  ...
  child.on('exit', code => {
    if (code !== 0) {
      ...
      const isPortOrPerm = err.includes('failed to start server') || err.includes('listen eperm') || err.includes('eaddrinuse') || err.includes('operation not permitted');
      if (isPortOrPerm) {
        console.log('\nℹ️  Skipping E2E: dev server cannot start in this environment.');
        process.exit(0);
        return;
      }
      if (err.includes('browserType.launch') || err.includes('please install browsers')) {
        console.log('\nℹ️  Playwright browsers not installed. Skipping E2E tests.');
        process.exit(0);
        return;
      }
      console.error('\n❌ Playwright E2E tests failed.');
      console.error('   If this is a fresh environment, run: npx playwright install');
      process.exit(code);
      return;
    }
    process.exit(0);
