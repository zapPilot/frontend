...
describe("WalletCard", () => {
  ...
  describe("switch wallet button", () => {
    it("should show switch button for non-active wallet when owner", () => {
      const onSwitchWallet = vi.fn();

      render(<WalletCard {...defaultProps} onSwitchWallet={onSwitchWallet} />);

      expect(
        screen.getByRole("button", { name: /switch to wallet/i })
      ).toBeInTheDocument();
    });

    it("should not show switch button for active wallet", () => {
      const activeWallet = { ...mockWallet, isActive: true };
      const onSwitchWallet = vi.fn();
      ...
      expect(
        screen.queryByRole("button", { name: /switch to wallet/i })
      ).not.toBeInTheDocument();
    });

    it("should not show switch button when not owner", () => {
      const onSwitchWallet = vi.fn();
    ...
    it("should not show switch button when onSwitchWallet not provided", () => {
    ...
    it("should call onSwitchWallet with address when clicked", async () => {
      ...
      expect(onSwitchWallet).toHaveBeenCalledWith(mockWallet.address);
    });

    it("should have correct aria-label for switch button", () => {
      const onSwitchWallet = vi.fn();

      render(<WalletCard {...defaultProps} onSwitchWallet={onSwitchWallet} />);

      const button = screen.getByRole("button", {
        name: "Switch to wallet Main Wallet",
      });
