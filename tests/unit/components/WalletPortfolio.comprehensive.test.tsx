...
vi.mock("../../../src/components/ui/GlassCard", () => ({
  GlassCard: vi.fn(({ children }) => <div data-testid="glass-card">{children}</div>),
}));
  ...
  WalletMetrics: vi.fn(({ 
    ...
    <div data-testid="wallet-metrics">
      <div data-testid="total-value">
        {isLoading ? "Loading..." : error ? `Error: ${error}` : balanceHidden ? "****" : `$${totalValue}`}
      </div>
...
vi.mock("../../../src/components/wallet/WalletActions", () => ({
  WalletActions: vi.fn(({ onZapInClick, onZapOutClick, onOptimizeClick }) => (
    <div data-testid="wallet-actions">
      ...
      <button data-testid="zap-out-button" onClick={onZapOutClick}>Zap Out</button>
      <button data-testid="optimize-button" onClick={onOptimizeClick}>Optimize</button>
    </div>
  )),
}));
...
vi.mock("../../../src/components/PortfolioOverview", () => ({
  PortfolioOverview: vi.fn(({ 
    ...
    <div data-testid="portfolio-overview">
      ...
      <div data-testid="portfolio-loading">{isLoading ? "loading" : "not-loading"}</div>
      <div data-testid="portfolio-error">{apiError || "no-error"}</div>
      <div data-testid="portfolio-retrying">{isRetrying ? "retrying" : "not-retrying"}</div>
      <div data-testid="portfolio-data-count">{portfolioData ? portfolioData.length : 0}</div>
      <div data-testid="pie-chart-data-count">{pieChartData ? pieChartData.length : 0}</div>
      <div data-testid="expanded-category">{expandedCategory || "none"}</div>
      <div data-testid="balance-hidden">{balanceHidden ? "hidden" : "visible"}</div>
      <div data-testid="total-value-prop">{totalValue !== undefined ? totalValue : "undefined"}</div>
      {onRetry && (
  ...
  WalletManager: vi.fn(({ isOpen, onClose }) => 
    isOpen ? (
      <div data-testid="wallet-manager">
        ...
        <button data-testid="close-wallet-manager" onClick={onClose}>Close</button>
      </div>
    ) : null
  ),
}));
...
describe("WalletPortfolio - Comprehensive Integration Tests", () => {
  ...
  describe("Hook Integration and State Management", () => {
    it("should handle loading states correctly", () => {
      ...
      expect(screen.getByTestId("portfolio-loading")).toHaveTextContent("loading");
    ...
    it("should handle error states correctly", () => {
      ...
      expect(screen.getByTestId("total-value")).toHaveTextContent(`Error: ${errorMessage}`);
      expect(screen.getByTestId("portfolio-error")).toHaveTextContent(errorMessage);
      expect(screen.getByTestId("portfolio-loading")).toHaveTextContent("not-loading");
    });
    ...
    it("should handle retry functionality", () => {
      ...
      expect(screen.getByTestId("portfolio-retrying")).toHaveTextContent("retrying");
    ...
    it("should handle balance visibility toggle", () => {
      ...
      expect(screen.getByTestId("toggle-balance-button")).toHaveTextContent("Show Balance");
  ...
  describe("Data Transformation and Edge Cases", () => {
    ...
    it("should handle null/undefined data gracefully", () => {
      ...
      expect(screen.getByTestId("portfolio-data-count")).toHaveTextContent("0");
      expect(screen.getByTestId("pie-chart-data-count")).toHaveTextContent("0");
      expect(screen.getByTestId("total-value-prop")).toHaveTextContent("undefined");
    });
    ...
    it("should handle large portfolio values", () => {
      ...
      expect(screen.getByTestId("total-value")).toHaveTextContent("$999999999.99");
    });
    ...
    it("should handle negative portfolio values", () => {
      ...
      expect(screen.getByTestId("change-percentage")).toHaveTextContent("-15.8%");
    });
  });
  ...
  describe("Error Boundary and Recovery", () => {
    ...
    it("should recover from API errors when data becomes available", () => {
      ...
      expect(screen.getByTestId("portfolio-error")).toHaveTextContent("API Error");

      // Simulate recovery
      mockUseWalletPortfolioState.mockReturnValue(defaultMockState);
      rerender(<WalletPortfolio />);

      expect(screen.getByTestId("portfolio-error")).toHaveTextContent("no-error");
      expect(screen.getByTestId("total-value")).toHaveTextContent("$15000");
    });
  });
  ...
  describe("Integration with External Systems", () => {
    ...
    it("should handle modal interactions correctly", async () => {
      ...
      expect(defaultMockState.toggleCategoryExpansion).toHaveBeenCalledWith("test-category");
    });
  });
  ...
  describe("Edge Cases and Stress Tests", () => {
    ...
    it("should handle extreme data values", () => {
      ...
      expect(screen.getByTestId("total-value")).toHaveTextContent(`$${Number.MAX_SAFE_INTEGER}`);
    });

    it("should handle portfolio data with many categories", () => {
      const largeMockData: AssetCategory[] = Array.from({ length: 100 }, (_, i) => ({
        id: `${i}`,
        name: `Asset${i}`,
        totalValue: i * 100,
        percentage: 1,
      ...
      const largePieData: PieChartData[] = Array.from({ length: 100 }, (_, i) => ({
        label: `Asset${i}`,
        value: i * 100,
        percentage: 1,
        color: `#${i.toString(16).padStart(6, '0')}`,
      ...
      expect(screen.getByTestId("portfolio-data-count")).toHaveTextContent("100");
      expect(screen.getByTestId("pie-chart-data-count")).toHaveTextContent("100");
    });
