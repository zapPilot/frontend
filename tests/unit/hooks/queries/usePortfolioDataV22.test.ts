/**
 * Unit tests for usePortfolioDataV22.ts
 *
...
vi.mock("@/adapters/portfolioDataAdapter", () => ({
  transformToV22DataWithDirection: vi.fn(),
}));

describe("usePortfolioDataV22", () => {
  ...
  const createMockV22Data = (): V22PortfolioDataWithDirection => ({
    balance: 50000,
    roi: 12.4,
    roiChange7d: 2.1,
    roiChange30d: 8.3,
  ...
  describe("parallel query execution", () => {
    it("should fetch landing, sentiment, and regime data in parallel", async () => {
      ...
      const mockV22Data = createMockV22Data();
      ...
      vi.mocked(transformToV22DataWithDirection).mockReturnValue(mockV22Data);

      const { result } = renderHook(() => usePortfolioDataV22("0xUSER123"), {
        wrapper,
      });
      ...
      expect(transformToV22DataWithDirection).toHaveBeenCalledWith(
        mockLanding,
        mockSentiment,
        mockRegime
      );

      expect(result.current.data).toEqual(mockV22Data);
  ...
  describe("loading states", () => {
    ...
    it("should not be loading when only sentiment data is loading", () => {
      ...
      vi.mocked(transformToV22DataWithDirection).mockReturnValue(
        createMockV22Data()
      );
  ...
  describe("error handling", () => {
    ...
    it("should NOT return error when sentiment data fails", () => {
      ...
      expect(result.current.error).toBeNull();
      expect(result.current.data).toBe(mockV22Data);
    });
  ...
  describe("data transformation", () => {
    it("should transform data with all three sources available", () => {
      ...
      renderHook(() => usePortfolioDataV22("0xUSER123"), { wrapper });
  ...
  describe("edge cases", () => {
    it("should return null data when landing data is not available", () => {
      ...
      expect(result.current.data).toBeNull();
      expect(transformToV22DataWithDirection).not.toHaveBeenCalled();
    });
    ...
    it("should handle rapid userId changes", () => {
      ...
      const { rerender } = renderHook(
        ({ userId }) => usePortfolioDataV22(userId),
        {
          wrapper,
