...
test.describe("Strategy View - Sub-tab Navigation", () => {
  ...
  test("switches between Suggestion and Backtesting sub-tabs", async ({
    ...
    await expect(
      page.getByRole("heading", { name: "Today's Strategy" })
    ).toBeVisible();
  ...
  test("shows loading state while fetching suggestion data", async ({
    ...
    await expect(page.getByText("Analyzing market regime...")).toBeVisible();

    // Wait for data to load
    await expect(page.getByText("Today's Strategy")).toBeVisible({
      timeout: 5000,
    });
  });
});
