/**
 * React Query Hook for Wallet Portfolio Data
 *
 * Fetches and transforms portfolio data for the wallet portfolio layout.
...
/**
 * Custom hook for fetching wallet portfolio data
 *
 * Combines data from:
 ...
 * @returns Portfolio data transformed for the wallet portfolio layout with loading/error states
 *
 * @example
 * ```typescript
 ...
 *
 * return <WalletPortfolioPresenter data={data} />;
 * ```
 */
export function usePortfolioData(userId: string) {
  ...
  const data: WalletPortfolioDataWithDirection | null = landingQuery.data
    ? transformToWalletPortfolioDataWithDirection(
        landingQuery.data,
