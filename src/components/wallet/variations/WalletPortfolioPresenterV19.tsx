...
import { ArrowDownCircle, ArrowUpCircle, ArrowUpRight, History, LayoutDashboard, LineChart, PieChart as PieChartIcon, Wallet, Zap } from "lucide-react";
...
export function WalletPortfolioPresenterV19() {
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col font-sans selection:bg-purple-500/30">
      ...
      <nav className="h-16 border-b border-gray-800/50 bg-gray-950/80 backdrop-blur-md sticky top-0 z-50 px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
           {/* Removed redundant balance display */}
      ...
      <main className="flex-1 flex items-center justify-center p-4 md:p-8">
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-6 min-h-[600px]">
          ...
          <div className="flex flex-col gap-6 h-full">
            ...
            {/* Regime Context Card (Simplified - No Allocation) */}
            <div className="bg-gray-900/40 backdrop-blur-sm border border-gray-800 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group">
               ...
               {/* Removed Target Allocation from here */}
            </div>
          </div>

          {/* RIGHT COLUMN: THE QUANTUM BAR + TARGET COMPARISON */}
          <div className="bg-gray-900/20 border border-gray-800 rounded-2xl p-8 flex flex-col relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-12">
              <div>
                ...
                <p className="text-sm text-gray-400">Target vs. Actual Allocation</p>
              </div>
            ...
            <div className="flex-1 flex flex-col justify-center gap-8">
               ...
               {/* 1. TARGET ROW */}
               <div>
                 <div className="flex justify-between text-xs text-gray-500 mb-2 uppercase tracking-widest font-bold">
                   <span>Target Strategy ({currentRegime.label})</span>
                 </div>
                 <div className="h-12 w-full bg-gray-900/30 rounded-xl border border-gray-800/50 flex overflow-hidden opacity-60 grayscale hover:grayscale-0 transition-all">
                    <div 
                      className="h-full flex items-center justify-center text-xs font-bold text-white/50 border-r border-gray-900/50"
                      style={{ width: `${currentRegime.allocation.crypto}%`, backgroundColor: currentRegime.fillColor }}
                    >
                      Crypto {currentRegime.allocation.crypto}%
                    </div>
                    <div 
                      className="h-full flex items-center justify-center text-xs font-bold text-emerald-400/50 bg-emerald-900/20"
               ...
               {/* 2. ACTUAL ROW (The Quantum Bar) */}
               <div>
                 <div className="flex justify-between text-xs text-white mb-2 uppercase tracking-widest font-bold">
                   <span>Current Allocation</span>
                   <span className="text-orange-400">Drift: {MOCK_DATA.delta}%</span>
                 </div>
                 
                 <div className="relative h-24 w-full bg-gray-900/50 rounded-2xl border border-gray-800 p-2 flex gap-2 shadow-2xl">
                    {/* CRYPTO SECTION */}
                    <div 
               ...
               {/* Legend */}
               <div className="flex gap-6 text-xs text-gray-400 px-1">
                  {MOCK_DATA.currentAllocation.simplifiedCrypto.map(asset => (
