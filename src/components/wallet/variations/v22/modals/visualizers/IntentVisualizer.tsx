...
export interface VisualizerLane {
  id: string;
  name: string;
  est: string;
}
...
export function IntentVisualizer({ 
  ...
  const [laneProgress, setLaneProgress] = useState<{ [key: string]: number }>({});

  useEffect(() => {
    // Reset state on mount or when props change
    const initialProgress = lanes.reduce((acc, lane) => ({ ...acc, [lane.id]: 0 }), {});
  ...
  return (
    <div className="w-full py-4 space-y-3">
        {lanes.map((lane) => {
            ...
            return (
                <div key={lane.id} className="bg-gray-900/50 border border-gray-800 rounded-lg p-3 flex items-center gap-4 relative overflow-hidden">
                    {/* Success Background Flash */}
                    {isComplete && <div className="absolute inset-0 bg-green-500/5 pointer-events-none" />}
                    
                    {/* Protocol Identity */}
                    <div className="w-8 h-8 rounded shrink-0 relative">
                        <img src={getProtocolLogo(lane.id)} className="w-full h-full object-cover rounded opacity-80" />
                        {isComplete && (
                            ...
                            return (
                                <div key={idx} className="relative z-10 flex flex-col items-center gap-1 group">
                                    <div className={`
                                        ...
                                        ${isStepComplete ? "bg-green-500 border-green-500" : 
                                          isStepActive ? "bg-gray-900 border-green-500 animate-pulse" : "bg-gray-950 border-gray-800"}
                                    `}></div>
                                    <span className={`text-[9px] font-bold uppercase tracking-wider ${isComplete || isStepComplete || isStepActive ? "text-gray-300" : "text-gray-700"}`}>
                                        {label}
                                    </span>
                                </div>
                            )
