...
export function WithdrawModal({
  ...
  const {
    ...
    selectedChain,
    isSubmitting,
  } = modalDeps.useTransactionModalState({
    isOpen,
    isConnected,
    onClose,
    defaultChainId,
  ...
  const handlePercentage = (pct: number) => {
    const max = parseFloat(
      transactionData.balances[transactionData.selectedToken?.address || ""]
        ?.balance || "0"
    );
    ...
    <Modal isOpen={isOpen} onClose={resetState} maxWidth="md">
      <ModalContent className="p-0 overflow-visible bg-gray-950 border-gray-800">
        <modalDeps.TransactionModalHeader
          ...
          indicatorClassName="bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
