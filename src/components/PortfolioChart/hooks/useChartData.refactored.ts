...
export function useChartData(
  ...
  const apiPortfolioHistory: PortfolioDataPoint[] = useMemo(() => {
    ...
    return dashboard.trends.daily_values.map(dailyValue => ({
      ...
      protocols: Object.entries(dailyValue.protocols || {}).map(([protocol, value]) => ({
        protocol,
  ...
  const rollingSharpeData = useMemo(() => {
    if (!dashboard?.rolling_analytics?.sharpe) return null;
    return {
      rolling_sharpe_data: dashboard.rolling_analytics.sharpe.rolling_sharpe_timeseries,
    };
  }, [dashboard?.rolling_analytics?.sharpe]);

  const rollingVolatilityData = useMemo(() => {
    if (!dashboard?.rolling_analytics?.volatility) return null;
    return {
      rolling_volatility_data: dashboard.rolling_analytics.volatility.rolling_volatility_timeseries,
    };
  }, [dashboard?.rolling_analytics?.volatility]);

  // Extract drawdown data
  const enhancedDrawdownData = useMemo(() => {
    if (!dashboard?.drawdown_analysis?.enhanced) return null;
    return {
      drawdown_data: dashboard.drawdown_analysis.enhanced.daily_drawdowns.map(d => ({
        date: d.date,
  ...
  const underwaterRecoveryData = useMemo(() => {
    if (!dashboard?.drawdown_analysis?.underwater_recovery) return null;

    // Transform underwater_periods to underwater_data format
    const underwaterData = dashboard.drawdown_analysis.underwater_recovery.underwater_periods.flatMap(period => {
      // Generate daily points for each underwater period
      // For now, we'll create at least start and end points
      const points = [];
      points.push({
  ...
  const hasPartialFailures = dashboard?._metadata?.error_count ?? 0 > 0;
  if (hasPartialFailures && dashboard?._metadata?.errors) {
    console.warn(
      "[useChartData] Some dashboard metrics failed to load:",
      dashboard._metadata.errors
    );
  }
