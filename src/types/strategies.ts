...
import { AssetCategory, Protocol } from "../components/PortfolioAllocation/types";
...
const matchesCategory = (pool: PoolDetail, categoryId: string): boolean => {
  ...
  switch (categoryId) {
    ...
    case 'eth':
      return symbols.some(s => ['eth', 'weth', 'steth', 'reth', 'ethereum'].includes(s));
    case 'stablecoins':
    case 'stablecoin':
      return symbols.some(s => ['usdc', 'usdt', 'dai', 'busd', 'frax', 'lusd'].includes(s));
    case 'others':
    default:
      // If it doesn't match BTC, ETH, or stablecoins, it goes to others
      const isBtc = symbols.some(s => ['btc', 'wbtc', 'bitcoin'].includes(s));
      const isEth = symbols.some(s => ['eth', 'weth', 'steth', 'reth', 'ethereum'].includes(s));
      const isStable = symbols.some(s => ['usdc', 'usdt', 'dai', 'busd', 'frax', 'lusd'].includes(s));
...
const formatProtocolName = (pool: PoolDetail): string => {
  const poolTokens = pool.pool_symbols.length > 0
    ? ` (${pool.pool_symbols.join('/')})`
    : '';
...
export const transformCategory = (
  ...
  // Transform real pool data to protocols for this category
  protocols: transformPoolsToProtocols(poolDetails, strategy.id),
});
